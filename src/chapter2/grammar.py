...
class CFG(Grammar):
    ...
    def parse(self):
        """
        仅考虑 非终结符 | 终结符 都是单个字符的情况, 更复杂的情况留到 BNF 解析时去考虑
        """
        # 产生式左侧的是非终结符
    ...
    def eliminate_left_recursion(self):
        """
        消除左递归: 优先消除间接左递归, 因为间接左递归消除的时候会带来新引入的直接左递归
        """
        self.eliminate_indirect_left_recursion()  # 间接左递归
        self.eliminate_direct_left_recursion()  # 直接左递归

    def eliminate_indirect_left_recursion(self):
        """
        消除间接左递归

        判断方法是如果一个非终结符在产生式左侧出现了, 然后它又在其他产生式的右侧开头位置出现了
        则说明有冲突
        ...
        """
        self.calculate_first_set()  # 先计算 first 集
                ...
                for char in production_body:
                    if char in self.terminal_symbols:
                        ...
                        if char in existed_symbols:
                            if extend_symbols.get(production_head) is not None:  # pragma: no cover
                                raise ValueError(f"{production_head} 出现了过多间接左递归依赖, 请重写文法")
                            if char in extend_symbols.keys():  # pragma: no cover
                                raise ValueError(f"{char} 出现了多级间接左递归依赖, 过于复杂请重写文法")
                            if char == production_head:
                                # 交给直接左递归消除 去处理
                                break
                            print(
                                f"[{char} {production_head} 存在间接左递归]: {production_head} -> {production_body}"
                            )
    ...
    def eliminate_direct_left_recursion(self):
        ...
        for production_head, production_bodys in self.productions.items():
            ...
            if flag:
                ...
                new_symbol = self.register_new_symbol()
    ...
    def _calculate_first_set(self, first_set:dict[str,set]) -> set:
        """
        递归的调用, 因为firset set 会在每一轮计算中不断变化, 所以需要反复计算 first set 直到不再变化
        """
    ...
    def register_long_non_terminal_symbol(self, symbol: str):
        """
        当
        """

    def register_new_symbol(self):
        """选择一个新的非终结符修正原文法"""

        for i in range(ord("A"), ord("Z") + 1):
            new_symbol = chr(i)
            if (
                new_symbol not in self.non_terminal_symbols
                and new_symbol not in self.terminal_symbols
            ):
